...
internal sealed class DispatchedObservableCollection<T> : ObservableCollectionBase<T>, IReadOnlyObservableCollection<T>, IList<T>, IList
{
    ...
    internal void EnqueueReset(System.Collections.Immutable.ImmutableList<T> items) => EnqueueEvent(PendingEvent.Reset(items));

    internal void EnqueueAdd(T item) => EnqueueEvent(PendingEvent.Add(item));

    internal void EnqueueAddRange(System.Collections.Immutable.ImmutableList<T> items) => EnqueueEvent(PendingEvent.AddRange(items));
    ...
    internal void EnqueueInsertRange(int index, System.Collections.Immutable.ImmutableList<T> items) => EnqueueEvent(PendingEvent.InsertRange(index, items));
    ...
    private bool IsOnDispatcherThread() => _dispatcher.Thread == Thread.CurrentThread;
